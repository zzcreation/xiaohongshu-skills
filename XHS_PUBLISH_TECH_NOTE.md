...
5. 多标签连续点击联想项时，日志可能显示全部成功，但预览里实际只保留第一个话题标签。
...
新版 Tiptap 编辑器中，连续为多个标签执行「输入 `#标签` → 点击联想第一项」并不可靠：

- CLI 日志可能显示每个标签都“点击标签联想”成功。
- 但右侧笔记预览/编辑器实际只保留第一个正式话题节点。
- 后续标签会被焦点切换、联想浮层或编辑器状态吞掉，形成“假成功”。

当前稳定策略：

1. 第一个标签仍走正式话题联想，生成平台话题节点。
2. 后续标签一次性追加为普通 hashtag 文本，例如：

```text
 #周末去哪儿 #遛娃好去处 #亲子乐园 #文旅项目
```

3. 填写完成后必须读取编辑器正文或右侧「笔记预览」做二次校验，不能只相信点击日志。

建议校验项：

- 编辑器正文中 `#[^\s#]+` 数量符合预期。
- 右侧「笔记预览」里能看到完整标签行。
- `xhs-publish-btn` 的 `submit-disabled="false"`。
...
- `_input_tags()`：第一个标签点击联想生成正式话题节点，后续标签通过 `_append_plain_tags()` 稳定追加普通 hashtag 文本，避免多标签“日志成功但预览缺失”的假成功。

## 已验证

- 连续 3 次稳定进入「上传图文」页。
- 完整发布测试通过：上传图片、填写标题正文、点击 ShadowRoot 内真实发布按钮，2 秒内检测到 `发布成功`。
- 预览流程测试通过：6 张图片、标题/正文、16 个标签在右侧「笔记预览」中可见；发布按钮可用但未点击发布。
